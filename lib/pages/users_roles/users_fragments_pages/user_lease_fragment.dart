...
class UserLeaseFragment extends StatefulWidget {
  UserLeaseFragment({super.key});

  @override
  State<UserLeaseFragment> createState() => _UserLeaseFragmentState();
}

class _UserLeaseFragmentState extends State<UserLeaseFragment> {
  bool isLoading = true;
  File? _documentFile;
  String _amount = '';
  String _frequency = '';
  ...
  void viewLeaseData(int leaseId) {
    for(var myLease in leases) {
      if(myLease.id == leaseId) {
        // Show dialog with lease details
        showDialog(
            context: context,
            builder: (context) {
              return AlertDialog(
  ...
  //             elevation: 5,
  //             title: const Text("Update Lease", style: TextStyle(fontWeight: FontWeight.bold)),
  //             content: Container(
  ...
  //                       children: [
  //                         // First Name
  //                         _buildTextField(context, _updateStartDateController, 'Start Date', width: MediaQuery.of(context).size.width * 0.31),
  //                         // Last Name
  //                         _buildTextField(context, _updateEndDateController, 'End Date', width: MediaQuery.of(context).size.width * 0.31),
  //                       ],
  //                     ),
  //
  ...
  //                       children: [
  //                         // Duration
  //                         _buildTextField(context, _updateDurationController, 'Duration',width: MediaQuery.of(context).size.width * 0.31),
  //
  //                         Container(
  //                           child: Column(
  //                             children: [
  ...
  //
  //                                       // file icon
  //                                       const Icon(Icons.folder, color: Colors.blue, size: 22,),
  //
  //                                       const SizedBox(width: 10,),
  //
  //                                       // name
  //                                       Text(
  //                                         myLease.document.toString(),
  //                                       ),
  //
  //                                     ],
  //                                   ),
  ...
  //                     _selectedUnity.toString(),
  //                     displayDiffInMonths(_updateStartDateController.text, _updateEndDateController.text).toString(), // return int numbers
      ...
      if (myLease.id == leaseId) {
        showDialog(
          ...
          builder: (context) {
            ...
            final _updateEndDateController = TextEditingController(text: myLease.endDate);
            final _updateDurationController = TextEditingController(text: myLease.frequency);
            final TextEditingController _updateDocumentNameController = TextEditingController();

            // _updateDocumentNameController.text = myLease.document;
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // _buildDocumentPicker(context, _updateDocumentNameController),

                            // GestureDetector(
                            //   onTap: _documentPickFile,
                            //   child: Container(
                            //     padding: const EdgeInsets.all(10),
                            //     decoration: BoxDecoration(
                            //       borderRadius: BorderRadius.circular(10),
                                  ...
                                  children: [
                                    ...
                                    ValueListenableBuilder<TextEditingValue>(
                                      ...
                                      builder: (context, value, child) {
                                        return Text(
                                          _pickedFileName.isNotEmpty ? _updateDocumentNameController.text : myLease.document,
                                          style: const TextStyle(color: Colors.black87),
                                        );
                                      },
                                    ),
  ...
  Widget _buildDocumentPicker(
    ...
    return GestureDetector(
      onTap: () async {
        ...
        // Update the document in the state and model
        setState(() {
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            const Icon(Icons.folder, color: Colors.blue, size: 32),
            const SizedBox(width: 10),
            Text(controller.text.isNotEmpty ? controller.text : documentName),
          ],
        ),
      ),
    );
