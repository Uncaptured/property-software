...
class UserTenantFragment extends StatefulWidget {
  UserTenantFragment({super.key});

  @override
  State<UserTenantFragment> createState() => _UserTenantFragmentState();
}

class _UserTenantFragmentState extends State<UserTenantFragment> {
  bool isLoading = true;
  ...
  void _onPropertyChanged(AllPropertyModel? selectedProperty) {
    setState(() {
      ...
      // Enable/disable the unity dropdown based on whether a property is selected
      _isUnityEnabled = selectedProperty != null;

      // Enable/disable the company name field based on the property type
      ...
      // Reset the selected Unity if it doesn't belong to the newly selected property
    ...
    for (var myTen in tenants) {
      if (myTen.id == tenantId) {
        showDialog(
          context: context,
          builder: (context) {
            // Initialize controllers with tenant data
            ...
            // Find the selected unity and property based on tenant's unity_id and property_id
            _selectedUnity = unities.firstWhereOrNull((unit) => myTen.unity_id == unit.id);
            _selectedProperty = properties.firstWhereOrNull((myProp) => myProp.id == _selectedUnity?.property_id);

            // Set the initial state based on the selected property and unity
            _isUnityEnabled = _selectedProperty != null;
            ...
            return AlertDialog(
              ...
              elevation: 5,
              title: const Text("Update Tenant", style: TextStyle(fontWeight: FontWeight.bold)),
              content: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          // First Name
                          _buildTextField(context, _updateFNameController, 'First Name', width: MediaQuery.of(context).size.width * 0.31),
                          // Last Name
                          _buildTextField(context, _updateLNameController, 'Last Name', width: MediaQuery.of(context).size.width * 0.31),
                        ],
                      ),
                      ...
                      // Email Address
                      _buildTextField(context, _updateEmailController, 'Email Address', width: MediaQuery.of(context).size.width * 0.63),
                      ...
                      Wrap(
                        ...
                        children: [
                          // Phone Number
                          _buildTextField(context, _updatePhoneController, 'Phone Number',width: MediaQuery.of(context).size.width * 0.31),
                          ...
                          // Company Name
                          _buildTextField(context, _updateCNameController, 'Company Name', width: MediaQuery.of(context).size.width * 0.31, enabled: _isCompanyNameEnabled),
                        ],
                      ),
      ...
      child: DropdownButtonFormField<AllUnityModel>(
        ...
        onChanged: _isUnityEnabled
          ...
          setState(() {
            ...
            : null,
        items: _filteredUnities.map((AllUnityModel unit) => DropdownMenuItem<AllUnityModel>(
          value: unit,
